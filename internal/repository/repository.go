...
func (r *repo) SaveGame(numRounds int, closesAt time.Time, createdBy string) (*domain.Game, error) {
	...
	err = stmt.Get(&adminId, map[string]interface{}{
		"created_by": createdBy,
	})
	if err != nil {
		log.Printf("Admin not found or scan failed: %v", err)
		tx.Rollback()
	...
	_, err = tx.Exec(`INSERT INTO games(id, num_rounds, created_at, closes_at, completion_status, created_by) VALUES ($1, $2, $3, $4, $5, $6)`,
		newGame.Id,
		newGame.NumRounds,
		newGame.CreatedAt,
		newGame.ClosesAt,
